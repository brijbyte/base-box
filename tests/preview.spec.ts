...
test("renders React + TS sample via esbuild transform + esm.sh import map (WebKit)", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator("#status")).toContainText("synced", { timeout: 15000 });

  const frame = page.frameLocator("#preview");
  await expect(frame.locator("h1")).toHaveText("base-box ⚡️", { timeout: 20000 });
...
test("strips TS types & resolves relative imports (no network)", async ({ page }) => {
...
test("live edit updates the preview", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator("#status")).toContainText("synced", { timeout: 15000 });
  await expect(page.frameLocator("#preview").locator("h1")).toBeVisible({ timeout: 20000 });
...
export function App() {
  ...
  await expect(page.frameLocator("#preview").locator("h1")).toHaveText("Edited 0", {
    timeout: 15000,
  });
});
